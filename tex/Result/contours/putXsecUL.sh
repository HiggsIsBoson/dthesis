printXsecUL(){
    ...
    ncont=`(cd ../../../; ls figures/Result/xsecUL/*${gridName}*.pdf | wc -l)`
    width=0.98
    ...
    if [ `cat directModels.list | grep ${gridName}` ]; then
	echo "    \\caption{Upperlimit of excluded cross-section (95$\\%$ CL) for model \\textbf{${gridName}}.  \\label{ fig::Result::exclLimit::${gridName} } }"
    else
	echo "    \\caption{Upperlimit of excluded cross-section (95$\\%$ CL) for model \\textbf{${gridName}} for the grids ${cap}.  \\label{ fig::Result::exclLimit::${gridName} } }"
    fi
...
out=../xsecUL_nonBenchMark.tex

echo "\\subsection{Cross-section Upper Limits for All Benchmark Models} \\label{ sec::Result::xsecUL::nonBenchMark }" | tee ${out}
for model in `cat onelepModels_Impl.list`; do
#for model in `cat test.list`; do
    printXsecUL $model | tee -a  ${out}
done
